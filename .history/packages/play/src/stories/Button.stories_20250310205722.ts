	...
	play: async ({
		...
		await step(
			...
			async () => {
				...
				console.log('Args after setting useThrottle to false:', args); // 添加调试日志
				await userEvent.tripleClick(btn);

				console.log('onClick call count:', args.onClick.mock.calls.length); // 添加调试日志
