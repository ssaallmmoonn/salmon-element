...
defineOptions({ name: 'YisFormItem' });
...
const isRequired = computed(
	() => (!ctx?.hideRequiredAsterisk && some(itemRules.value, 'required')) || props?.required
);
...
const itemRules = computed(() => {
	...
	if (!isNil(required)) {
		...
		if (size(requiredRules)) {
			// 如果rules外层有required:true，而rules数组有些有required:true而有些没有，则需要给没有
			...
			// 如果没有 required 规则，添加新的 required 规则
			rules.push({ required });
		}
	}
...
<template>
	...
	>
		...
		<div class="yis-form-item__content">
			<slot :validate="validate"></slot>
			<div class="yis-form-item__error-msg" v-if="validateStatus === 'error'">
...
.yis-form-item {
	--yis-form-lebel-width: v-bind(normalizeLabelWidth) !important;
}
</style>
