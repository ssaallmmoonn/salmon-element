...
describe('Popconfirm/index.ts', () => {
	...
	it('should be exported with withInstall()', () => {
		expect(YisPopconfirm.install).toBeDefined();
	});

	// 测试 Popconfirm 组件是否被正确导出
	it('should be exported Popconfirm component', () => {
		expect(YisPopconfirm).toBe(PopConfirm);
	});
	...
	test('should enhance Popconfirm component', () => {
		...
		expect(enhancedPopconfirm).toBe(YisPopconfirm);
...
describe('Popconfirm.vue', () => {
	...
	test('popconfirm emits', async () => {
		...
		expect(wrapper.find('.yis-popconfirm').exists()).toBeTruthy();
		const confirmBtn = wrapper.find('.yis-popconfirm__confirm');
		expect(confirmBtn.exists()).toBeTruthy();

		confirmBtn.trigger('click');
		await vi.runAllTimers();
		expect(wrapper.find('.yis-popconfirm').exists()).toBeFalsy();
		...
		const cancelBtn = wrapper.find('.yis-popconfirm__cancel');
