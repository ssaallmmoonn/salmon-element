...
<template>
	<transition name="fade-in-linear" @after-leave="destroy">
		<yis-overlay v-show="(visible as Ref).value" :z-index="state.zIndex" mask>
			<div role="dialog" class="yis-overlay-message-box" @click="handleWrapperClick">
				<div
					ref="rootRef"
					:class="[
						'yis-message-box',
				...
				>
					<div
						...
						ref="headerRef"
						class="yis-message-box__header"
						:class="{ 'show-close': state.showClose }"
					>
						<div class="yis-message-box__title">
							<yis-icon
								v-if="iconComponent && state.center"
								:class="{
									[`yis-icon-${state.type}`]: state.type,
								}"
								:icon="iconComponent"
							/>
							{{ state.title }}
						</div>
						<button v-if="showClose" class="yis-message-box__header-btn" @click.stop="handleClose">
							<yis-icon icon="xmark" />
						</button>
					</div>
					<div class="yis-message-box__content">
						<yis-icon
							...
							:icon="iconComponent"
						/>
						<div v-if="hasMessage" class="yis-message-box__message">
							<slot>
								<component
					...
					<div v-show="state.showInput" class="yis-message-box__input">
						<yis-input
					...
					<div class="yis-message-box__footer">
						<yis-button
							v-if="state.showCancelButton"
							class="yis-message-box__footer-btn yis-message-box__cancel-btn"
						...
						<yis-button
							...
							class="yis-message-box__footer-btn yis-message-box__confirm-btn"
