...
import { useZIndex } from '@salmon-element/hooks';
import LoadingComp from './Loading.vue';
import { defer, delay, isNil, isString } from 'lodash-es';

const RELATIVE_CLASS = 'yis-loading-parent--relative' as const;
const HIDDEN_CLASS = 'yis-loading-parent--hiden' as const;
const LOADING_NUMB_KEY = 'yis-loading-numb' as const;

const instanceMap: Map<HTMLElement, LoadingInstance> = new Map();
const { nextZIndex } = useZIndex(3000);

// 创建一个加载实例，并返回控制该实例的方法
...
// 动态添加或移除目标元素的样式类，用于控制加载器的定位和内容显示
